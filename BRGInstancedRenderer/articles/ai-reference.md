...
- **Prototype prefabs should carry a `BRGPrototypeExtraData` component even with no overrides set.** Its presence is what makes the importer bake tight cull bounds into `PrototypeData.prefitCullBounds`; without it, bounds are measured during registration instead (slower, and needs a readable mesh). When building a `RenderSignature` by hand for procedural meshes, populate `asset.prefitCullBounds` yourself — one `PrefitCullBounds { centre, radius, cylRadius }` per LOD index, `radius > 0` to be valid. Config toggle **Allow Fallback Runtime Bounds Fitting** (formerly `fitCullBoundsToVertices`) governs the fallback path.
